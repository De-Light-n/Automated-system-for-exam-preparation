...
import { ArrowLeft, RotateCw, CheckCircle, XCircle, Clock, Check } from 'lucide-react';

interface FlashcardsProps {
  cards: Flashcard[];
  onComplete: (score: number) => void;
  onBack: () => void;
  onUpdateCard: (id: string, status: 'new' | 'learning' | 'mastered', nextReview: number) => void;
}

export const Flashcards: React.FC<FlashcardsProps> = ({ cards, onComplete, onBack, onUpdateCard }) => {
  ...
  const handleResponse = (known: boolean) => {
    ...
    const nextReview = known ? Date.now() + 24 * 60 * 60 * 1000 : Date.now() + 10 * 60 * 1000;
  ...
  if (completed) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center animate-in zoom-in duration-300">
        ...
        <h2 className="text-4xl font-heading font-bold text-slate-800 mb-2">Чудова робота!</h2>
        <p className="text-slate-600 mb-8 text-lg">
            Ти вивчив <span className="font-bold text-slate-900">{sessionMastered}</span> нових карток.
        </p>
        <button onClick={onBack} className="px-10 py-4 bg-slate-900 text-white rounded-2xl font-bold hover:bg-slate-800 shadow-xl transition-all hover:scale-105">
          Повернутися до меню
        </button>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl mx-auto py-4 px-4">
      <div className="flex justify-between items-center mb-8">
        <button onClick={onBack} className="text-slate-500 hover:text-primary flex items-center gap-2 transition-colors font-medium bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-100">
          <ArrowLeft className="w-4 h-4" /> Назад
        </button>
        <div className="flex flex-col items-end">
             <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Прогрес</span>
      ...
      <div className="perspective w-full h-[400px] cursor-pointer group relative" onClick={handleFlip}>
        <div className={`relative w-full h-full transition-all duration-700 preserve-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
          
          {/* Front */}
          <div className="absolute w-full h-full backface-hidden bg-white rounded-3xl shadow-[0_20px_50px_rgba(8,_112,_184,_0.07)] border border-slate-100 flex flex-col items-center justify-center p-10 text-center relative overflow-hidden">
            ...
            <span className="text-xs uppercase tracking-[0.2em] text-slate-400 font-bold mb-6 relative z-10">Питання</span>
          ...
          {/* Back */}
          <div className="absolute w-full h-full backface-hidden rotate-y-180 bg-slate-900 rounded-3xl shadow-2xl flex flex-col items-center justify-center p-10 text-center text-white border border-slate-700">
            <div className="absolute inset-0 opacity-20 bg-[radial-gradient(#ffffff_1px,transparent_1px)] [background-size:20px_20px]"></div>
            <span className="text-xs uppercase tracking-[0.2em] text-slate-400 font-bold mb-6 relative z-10">Відповідь</span>
            <p className="text-2xl leading-relaxed font-medium relative z-10">{currentCard.answer}</p>
          </div>
        </div>
      </div>
