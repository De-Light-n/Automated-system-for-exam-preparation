...
import { Check, X, ArrowRight, AlertCircle, Loader2, CheckCircle2, XCircle, Trophy, HelpCircle } from 'lucide-react';
...
export const Quiz: React.FC<QuizProps> = ({ content, onFinish, onBack }) => {
  ...
  const completeQuiz = () => {
      const result: QuizResult = {
        ...
        feedback: score > questions.length / 2 ? "Хороший результат!" : "Варто повторити матеріал.",
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh]">
        ...
        <p className="text-slate-800 font-heading font-bold text-xl">AI генерує тест...</p>
        <p className="text-slate-400 text-sm mt-2">Аналізуємо матеріал, формулюємо питання</p>
      </div>
    );
  }
  ...
  if (questions.length === 0) {
      return (
          <div className="text-center p-10">
              <p className="text-slate-600 mb-4">Не вдалося згенерувати тест. Спробуйте пізніше.</p>
              <button onClick={onBack} className="text-primary hover:underline">Назад</button>
          </div>
      )
  }
          ...
          <div className="max-w-3xl mx-auto py-8 px-4">
              <div className="glass rounded-3xl p-10 text-center mb-10 border border-white/50 shadow-xl">
                  ...
                  <h2 className="text-3xl font-heading font-bold text-slate-800 mb-2">
                      {percentage >= 80 ? "Відмінний результат!" : percentage >= 50 ? "Непогано!" : "Треба підучити"}
                  </h2>
              ...
              <div className="space-y-6 mb-10">
                  {questions.map((q, idx) => {
                      ...
                      return (
                          <div key={q.id} className={`p-6 rounded-2xl border-l-4 shadow-sm bg-white ${isCorrect ? 'border-l-green-500' : 'border-l-red-500'}`}>
                              <div className="flex gap-4">
                                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${isCorrect ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                                      {isCorrect ? <Check className="w-5 h-5" /> : <X className="w-5 h-5" />}
                                  </div>
                                  <div className="flex-1">
                                      <p className="font-bold text-slate-800 text-lg mb-2">{q.question}</p>
                                      <p className="text-slate-500 text-sm mb-3">
                                          Ваша відповідь: <span className={isCorrect ? 'text-green-600 font-bold' : 'text-red-600 font-bold line-through'}>{userAnswer}</span>
                                      </p>
                                      {!isCorrect && (
                                          <div className="p-3 bg-green-50 rounded-xl border border-green-100 text-green-800 text-sm">
                                              <span className="font-bold">Правильно:</span> {q.correctAnswer}
                                          </div>
                                      )}
              ...
              <div className="flex justify-center gap-4">
                  <button onClick={completeQuiz} className="px-10 py-4 rounded-2xl bg-slate-900 text-white font-bold hover:bg-slate-800 shadow-xl shadow-slate-900/20 transition-transform hover:scale-105">
                      Завершити тест
                  </button>
              </div>
          </div>
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
       <div className="flex items-center gap-4 mb-8">
            <button onClick={onBack} className="w-10 h-10 flex items-center justify-center rounded-full bg-white border border-slate-200 text-slate-500 hover:bg-slate-50">
                <X className="w-5 h-5" />
            </button>
            <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full bg-primary transition-all duration-700 ease-out" style={{ width: `${progress}%` }}></div>
            </div>
            <span className="font-bold text-slate-400 tabular-nums">{currentQIndex + 1}/{questions.length}</span>
       </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-8 md:p-10 min-h-[400px] flex flex-col relative overflow-hidden">
        ...
        <div className="mb-8">
             <span className="inline-block px-3 py-1 bg-slate-100 text-slate-500 rounded-full text-xs font-bold uppercase tracking-wider mb-4">
                {currentQ.type === 'multiple_choice' ? 'Оберіть варіант' : 
                 currentQ.type === 'true_false' ? 'Так чи Ні' : 
                 'Заповніть пропуск'}
             </span>
            <p className="text-2xl md:text-3xl font-heading font-bold text-slate-900 leading-tight">{currentQ.question}</p>
        </div>

        <div className="space-y-3 flex-1">
          {(currentQ.type === 'multiple_choice' || currentQ.type === 'true_false') && currentQ.options?.map((opt, idx) => {
             const isSelected = selectedOption === opt;
             const isCorrect = opt === currentQ.correctAnswer;
             
             let btnClass = "w-full text-left p-5 rounded-2xl border-2 transition-all duration-200 group relative overflow-hidden ";
             
             if (selectedOption) {
                 if (isSelected) {
                     btnClass += isCorrect ? "bg-green-50 border-green-500 text-green-800" : "bg-red-50 border-red-500 text-red-800";
                 } else if (isCorrect) {
                 ...
                 btnClass += "border-slate-100 hover:border-primary hover:bg-slate-50 text-slate-700 hover:shadow-md";
             }

             return (
                <button key={idx} onClick={() => handleMultipleChoice(opt)} disabled={!!selectedOption} className={btnClass}>
                  ...
                  <div className="relative">
                      <input
                          ...
                          className={`w-full p-5 text-lg rounded-2xl border-2 outline-none transition-all font-medium
                            ${isInputSubmitted 
                                ? textInput.trim().toLowerCase() === currentQ.correctAnswer.trim().toLowerCase()
                    ...
                    >
                      ...
                      textInput.trim().toLowerCase() !== currentQ.correctAnswer.trim().toLowerCase() && (
                          <div className="p-4 rounded-xl bg-green-50 border border-green-200 text-green-800 animate-in fade-in slide-in-from-top-2">
                              <p className="text-xs font-bold uppercase tracking-wider mb-1 opacity-70">Правильна відповідь</p>
                              <p className="text-lg font-bold">{currentQ.correctAnswer}</p>
                          </div>
                      )
                  )}
              </div>
